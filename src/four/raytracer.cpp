...
Vec3f mirrorDirection(const Vec3f& normal, const Vec3f& incoming) {
	...
	return (incoming - 2 * dot(normal, incoming) * normal).normalized();
}
...
Vec3f RayTracer::traceRay(Ray& ray, float tmin, int bounces, float refr_index, Hit& hit, FW::Vec3f debug_color) const {
	...
	answer = m->diffuse_color( point)* scene_.getAmbientLight();
	...
	Vec3f dir_to_light(0, 0, 0);
	Vec3f incident_intensity(0, 0, 0);
	float distance = 0.0;
	...
	for (int i = 0; i < scene_.getNumLights(); i++){
		...
		Ray r = Ray(ray.pointAtParameter(hit.t) - ray.direction*EPSILON, -dir_to_light);
		Hit y;
		if (args_.shadows){
			if (scene_.getGroup()->intersect(r, y, EPSILON) == 0){
	...
	if (bounces >= 1) {
		...
		if (m->reflective_color(point).length() > 0.0f) {
			...
			Ray ray = Ray(ray.pointAtParameter(hit.t) - EPSILON*ray.direction,
				mirrorDirection(normal, ray.direction));
			Hit y;
			answer += traceRay(ray, tmin, bounces - 1, refr_index, y, debug_color)*hit.material->reflective_color(point);
		}
