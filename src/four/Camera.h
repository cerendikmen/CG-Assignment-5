...
public:
	...
	static inline FW::Vec2f normalizedImageCoordinateFromPixelCoordinate(const FW::Vec2f& pixel, const FW::Vec2i& imageSize) {
		...
		FW::Vec2f screen;
		screen.x = 2 * (pixel.x / imageSize.x) - 1;
		screen.y = -(2 * (pixel.y / imageSize.y) - 1);
...
public:
	...
	virtual Ray generateRay(const FW::Vec2f& point) {
		...
		FW::Vec2f normalized;
		normalized.x = point.x / 2;
		normalized.y = point.y/ 2;
		

		FW::Vec3f origin = center + normalized.x * size*horizontal + normalized.y * size * up;
		FW::Vec3f dir = direction;
...
public:
	...
	{	
		...
		float d = 1 / tanf(fov_angle / 2.0);
		auto dir = (point.x * horizontal + point.y * up + d*direction).normalized();
